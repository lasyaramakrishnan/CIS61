...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Incorrect number of arguments: expected {0}, got {1}".format(len(formals), len(vals)))
...
def do_and_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    python_list = list(vals)  #convert scheme to python 
    if not python_list:
        return True  #empty 'and' expression
    for expr in python_list:
        result = scheme_eval(expr, env)
        if not scheme_true(result):
            return False
    return python_list[-1] 
...
def do_or_form(vals, env):
    ...
    python_list = list(vals)  #convert scheme to python
    if not python_list:
        return False  #empty 'or' expression
    for expr in python_list:
        result = scheme_eval(expr, env)
...
def check_formals(formals):
    ...
    def check_and_add_symbol(symbol):
        if not scheme_symbolp(symbol):
            raise SchemeError("Formals must be symbols: {0}".format(symbol))
        elif symbol in symbols_set:
            raise SchemeError("Repeated symbol in formals: {0}".format(symbol))
        symbols_set.add(symbol)

    while formals is not nil:
        symbol = formals.first
        check_and_add_symbol(symbol)
        formals = formals.second
