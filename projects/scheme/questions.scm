...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (construct-partitions remaining pieces)
    (cond ((= remaining 0) (list pieces))
          ((or (< remaining 0) (= pieces 0)) '())
          (else (append (construct-partitions (- remaining max-value) (- pieces 1))
                        (map (lambda (p) (cons max-value p))
                             (construct-partitions (- remaining 1) pieces)))))))
...
(define (tree-sums tree)
  ; *** YOUR CODE HERE ***
  (define (sums-helper node current-sum)
    (let ((value (entry node))
          (children-list (children node)))
      (if (null? children-list)
          (list (+ current-sum value))
          (apply append
                 (map (lambda (child)
                        (sums-helper child (+ current-sum value)))
                      children-list))))))
