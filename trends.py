...
def make_tweet_fn(text, time, lat, lon):
    ...
    tweet_info = {'text': text, 'time': time, 'latitude': lat, 'longitude': lon}
    def return_make_tweet_fn(key):
        nonlocal tweet_info
        if key == 'text':
            return tweet_info['text']
        elif key == 'time':
            return tweet_info['time']
        elif key == 'lat':
            return tweet_info['latitude']
        elif key == 'lon':
            return tweet_info['longitude']
        else: 
            pass 
    return return_make_tweet_fn
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    sentiment_list = list(sentiment_value(get_word_sentiment(word)) for word in tweet_words(tweet) if has_sentiment(get_word_sentiment(word))) 
    #start by creating a list of sentiments for each word in the tweet
    if not sentiment_list: #if no word in the tweet had sentiments 
        return make_sentiment(None) #then return a sentiment of None
 
    total = sum(sentiment_list)
    length = len(sentiment_list)
...
def find_state_center(polygons):
    ...
    for i in range(0, len(polygons)):
        area_polygon = area_polygon + find_centroid(polygons[i])[2]  #The formala for calculating the are of all polygons combined
