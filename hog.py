...
def select_dice(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
#QUESTION 3
    if (opponent_score + score) % 7 == 0:  #unless sum is multiple of 7 
        return four_sided   #in which case select 4 sided
    else:
        return six_sided #select 6 sided dice
...
#QUESTION 4
    while score < goal and opponent_score < goal:
        player = strategy0(opponent_score, score) 
        if select_dice(score, opponent_score) == four_sided: 
            dice = four_sided

        else:
            dice = six_sided
        player_current_score = take_turn(player, opponent_score, dice)
        score = score + player_current_score

        if score == opponent_score * 2 or opponent_score == 2 * score:
            score, opponent_score = opponent_score, score

        elif score >= 100:    #this was where it was erroring!! 
            return score, opponent_score
        player1 = strategy1(opponent_score, score)
        ...
        if select_dice(score, opponent_score) == four_sided: 
            ...
            dice = six_sided 
        current_score1 = take_turn(player1, score, dice)
...
#QUESTION 6
    ...
    for num_rolls in range(1, 11): #return the number of dice (1-10)
        average_score = make_averaged(roll_dice)(num_rolls, dice) #by calling roll_dice with provided dice
        print(str(num_rolls) + ' dice scores ' +  str(average_score) + ' on average') #note to self: try using the f function instead to prnt instead of str

        if average_score > highest_avg:
            highest_avg = average_score
            return_num_rolls = num_rolls

    print(return_num_rolls)
...
#QUESTION 8
    ...
    if (score + max_opp) * 2 == opponent_score:
        return 0 #beneficial
    if (score + max_opp) / 2 == opponent_score:
        return BASELINE_NUM_ROLLS  #harmful
    else: 
...
def final_strategy(score, opponent_score):
    ...
    *** The main part of my final strategy aims to achieve a high win rate against the always_roll(5). 
    I decided to focus on adjusting the number of dice rolls based on the game situation:
    A: The strategy takes fewer risks (rolls 2 dice) when the player is in the lead but below the conservative threshold. 
    B: The strategy takes bigger risks (rolls 8 dice) when the player is behind but above the attacking threshold. 
    For all other situations, my strategy focuses on rolling a medium number of dice (4 dice).***
    """
    "*** YOUR CODE HERE ***"
#QUESTION 9
    attack_threshold = 30  
    ...
    if score > opponent_score and score <= conservative_threshold:
        # Take fewer risks when in the lead
        return 2
    elif score < opponent_score and score >= attack_threshold:
        # Take bigger risks to catch up when behind
        return 8
    return 4 # Default strategy

    #The second part of my final strategy makes sure that the opponent has a 4 sided dice in their next turn, playing into the Hog wild special rule: 
    num_rolls = BASELINE_NUM_ROLLS
    max_opp = (int(max(str(opponent_score))) + 1)
    if (score + max_opp + opponent_score) % 7 == 0: 
        return 0
    if score + max_opp >= 100:
        return 0
